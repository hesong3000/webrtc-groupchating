...
public class StateIdle implements State {
    ...
    private boolean bLocalCapture = false;

    @Override
    public String startVideoCall(final StateContext context, Bundle params) {
        Log.d("webrtc","StateIdle invoke startVideoCall");
    ...
    public String stopVideoCall(StateContext context) {
        Log.d("webrtc","idle state do not support stopVideoCall");
        return stateError;
    }

    @Override
    public String startAudioCall(StateContext context, Bundle params) {
        Log.d("webrtc","StateIdle invoke startAudioCall");
    ...
    public String stopAudioCall(StateContext context) {
        Log.d("webrtc","idle state do not support stopAudioCall");
        return stateError;
    }

    @Override
    public String startAudioRecord(StateContext context, Bundle params) {
        Log.d("webrtc","StateIdle invoke startAudioRecord");
        if(context == null){
        ...
        //参数完整性校验b--------
        if ((!params.containsKey("recordDir"))||(!params.containsKey("recordFile"))) {
            Log.d("webrtc", "startAudioRecord lack of params");
            return stateError;
        }
        //参数完整性校验e--------
        ...
        //参数合理性校验b--------
        String recordDir = "";
        recordDir = params.getString("recordDir");
        String recordFile = "";
        recordFile = params.getString("recordFile");

        //检测文件夹合理性b--------
        File file = new File(recordDir);
        if(file.isDirectory()==false){
            Log.d("webrtc", "startAudioRecord recordDir:" + recordDir + " is invalid");
            return stateError;
        }

        if(file.exists()==false){
            Log.d("webrtc", "startAudioRecord recordDir:" + recordDir + " is not exists");
            return stateError;
        }

        if(file.canWrite()==false){
            Log.d("webrtc", "startAudioRecord recordDir:" + recordDir + " can not write");
            return stateError;
        }

        if((file.getFreeSpace()/(1024*1024))<R.integer.min_audio_record_dir_MB){
            Log.d("webrtc", "startAudioRecord recordDir:" + recordDir + " not have enough space");
            return stateError;
        }
        //检测文件夹合理性e--------
        //检测文件名合理性b--------
        if(recordFile.length()<=4){
            Log.d("webrtc", "startAudioRecord recordFile:" + recordFile + " is invalid");
            return stateError;
        }

        if(recordFile.substring(recordFile.length()-3).toLowerCase().equals(
                R.string.audio_recordFile_format)==false){
            Log.d("webrtc", "startAudioRecord recordFile:" + recordFile + " not support format");
            return stateError;
        }
        //检测文件名合理性e--------
        //参数合理性校验e--------
        String wholeRecordFile = recordDir+"\\"+recordFile;
        context.getEngine().startAudioRecord(wholeRecordFile);
        context.setCurrent(context.STATE_RECORD);
        return stateOK;
    }

    @Override
    public String stopAudioRecord(StateContext context) {
        Log.d("webrtc","idle state do not support stopAudioRecord");
        return stateError;
    }

    @Override
    public String startAudioPlayout(StateContext context, Bundle params) {
        Log.d("webrtc","StateIdle invoke startAudioPlayout");
        if(context == null){
            Log.d("webrtc","StateContext has non pointer");
            return stateError;
        }
        ...
        context.setCurrent(context.STATE_PLAYOUT);
        context.getCurrent().invokeStateInit(context);
        return stateOK;
    }
    ...
    public String stopAudioPlayout(StateContext context) {
        Log.d("webrtc","idle state do not support stopAudioPlayout");
        return stateError;
    }

    @Override
    public String setMicMute(StateContext context) {
        Log.d("webrtc","idle state do not support setMicMute");
        return stateError;
    }

    @Override
    public String resumeMicMute(StateContext context) {
        Log.d("webrtc","idle state do not support resumeMicMute");
        return stateError;
    }

    @Override
    public String setRenderMute(StateContext context) {
        Log.d("webrtc","idle state do not support setRenderMute");
        return stateError;
    }

    @Override
    public String resumeRenderMute(StateContext context) {
        Log.d("webrtc","idle state do not support resumeRenderMute");
        return stateError;
    }

    @Override
    public String switchCameraFacing(StateContext context) {
        Log.d("webrtc","idle state do not support switchCameraFacing");
        return stateError;
    }

    @Override
    public String switchRenderView(StateContext context) {
        Log.d("webrtc","idle state do not support switchRenderView");
        return stateError;
    }

    @Override
    public String switchToAudioCall(StateContext context) {
        Log.d("webrtc","idle state do not support switchToAudioCall");
        return stateError;
    }
    ...
    public String invokeStateInit(StateContext context) {
        Log.d("webrtc","StateIdle invoke invokeStateInit");
        bLocalCapture = false;
        return stateOK;
    }
    ...
    public String startLocalCapture(StateContext context) {
        Log.d("mysurface1","StateIdle invoke startLocalCapture");
        if(bLocalCapture==false) {
           if(context.getEngine().isRunning()==false) {
    ...
    public String stopLocalCapture(StateContext context) {
        Log.d("mysurface1","StateIdle invoke stopLocalCapture");
        if(bLocalCapture==true){
            context.getEngine().stopCamera();
            SurfaceView localSurfaceView = context.getEngine().getLocalSurfaceView();
            if(localSurfaceView!=null) {
    ...
    public String setLoudSpeakerOn(StateContext context) {
        Log.d("webrtc","idle state do not support setLoudSpeakerOn");
        return stateError;
    }

    @Override
    public String setLoudSpeakerOff(StateContext context) {
        Log.d("webrtc","idle state do not support setLoudSpeakerOff");
        return stateError;
    }
